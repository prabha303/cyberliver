...
func logAndGetContext(w http.ResponseWriter, r *http.Request) *RequestData {
	...
	cfg := log.NewConfig("ec-user")
	//cfg.SetRemoteConfig(config.LOG_REMOTE_URL, config.LOG_TOKEN, config.LOG_UNAME)
	//cfg.SetLevelStr(config.LOG_LEVEL)
	//cfg.SetFilePathSizeStr(config.LOG_FILE_PATH_SIZE)
